...
export class AgreementPage implements OnInit {
  ...
  saveLesson() {
    if (this.agreementLesson.order < this.userDialect.lesson.order) {
      this.router.navigate(['/tabs/lessons']);
    } else if (this.user.uid && this.agreementLesson) {
      this.authentificationService.updateLesson(this.agreementLesson, this.user.uid).then(() => {
