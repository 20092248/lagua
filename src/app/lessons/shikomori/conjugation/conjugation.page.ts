...
export class ConjugationPage implements OnInit {
  ...
  saveLesson() {
    if (this.conjugationLesson.order < this.userDialect.lesson.order) {
      this.router.navigate(['/tabs/lessons']);
    } else if (this.user.uid && this.conjugationLesson) {
      this.authentificationService.updateLesson(this.conjugationLesson, this.user.uid).then(() => {
